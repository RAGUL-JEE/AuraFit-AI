...
def generate_frames():
    ...
    while True:
        ...
        if camera is None or not camera.isOpened():
            ...
            cv2.putText(err_img, "Please check hardware.", (110, 260), 
        ...
        # Check active status to avoid unnecessary processing
        if not is_active:
            # Stream a standby graphic overlay when tracking is off
            # We still read the frame to clear the OpenCV buffer!
            ...
            yield (b'--frame\r\n'
                   b'Content-Type:image/jpeg\r\n\r\n' + buffer.tobytes() + b'\r\n')
            # Don't sleep here, or camera buffer will lag
            continue
...
def save_workout():
    req_json = request.json or {}
    try:
        # We need a new connection since we are in a new thread
        conn_local = sqlite3.connect("history.db", check_same_thread=False)
        cursor_local = conn_local.cursor()
        cursor_local.execute("INSERT INTO history (workout, reps, calories) VALUES (?, ?, ?)",
                       (req_json.get("workout", "unknown"),
                        req_json.get("reps", 0),
                        req_json.get("calories", 0.0)))
        conn_local.commit()
        conn_local.close()
